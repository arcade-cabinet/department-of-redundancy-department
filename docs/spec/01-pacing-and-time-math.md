...
2. **Always have at least one "calm beat" (background-shamble or civilian) per 3 high-pressure beats** to give the player a breath.
...
| Difficulty | Beats per combat position (avg) | Civilian density | Enemy HP |
|---|---|---|---|
| Easy | 2.0 | 1 in 5 positions | 0.8× |
| Normal | 2.5 | 1 in 4 positions | 1.0× |
| Hard | 3.0 | 1 in 3 positions | 1.2× |
| Nightmare | 3.5 | 1 in 2 positions | 1.4× |
| Ultra Nightmare | 4.0+ | every position | 1.6× |
...
## Per-level disposal discipline

Each level captures every Babylon resource it constructs (`Scene` / `Mesh` / `Material` / `Texture` / `AnimationGroup` / `Sound`) and disposes via `scene.dispose()` (which cascades) when the camera-rail's `transition` cue fires. The next level boots its own scene at `t=0` with its own freshly-loaded assets.

Shared assets — viewmodel hands, the two player weapons, the common enemy archetypes (`security-guard`, `middle-manager`, `hitman`, `swat`) — are loaded once at game-start and live in a long-running asset cache. Per-level exclusive assets (level-specific props, mini-boss / boss GLBs, level-specific texture overlays) are loaded on level-construct, disposed on level-end.

Per-level estimated GPU memory:
...
Pre-load the next level during the current level's last combat position; dispose the previous level's exclusive assets during the next level's stairway.
