...
Three-room executive floor: reception → lounge → Crawford's office. Mahogany walls, leather booths, panic-alarm strobing.

### Floors / ceilings

| id | kind | origin | size | PBR |
|---|---|---|---|---|
| `floor-reception` | floor | (0, 0, 4) | 8 × 8 | `laminate` (mahogany tint) |
| `floor-lounge` | floor | (0, 0, 12) | 12 × 8 | `laminate` (mahogany + red-carpet runner) |
| `floor-crawford-office` | floor | (0, 0, 22) | 12 × 8 | `laminate` (mahogany) |
| `ceiling-exec` | ceiling | (0, 3, 12) | 12 × 32 | `ceiling-tile`, height 3, 12 emissive cutouts; panic-strobe overlay tweens between (1,0,0) and (1,1,0.95) |

### Walls

| id | kind | origin | size | overlay |
|---|---|---|---|---|
| `wall-N` | wall | (-6, 0, 14) | 32 × 3 | drywall (mahogany wainscoting) |
| `wall-S` | wall | (6, 0, 14) | 32 × 3 | drywall (mahogany) |
| `wall-lounge-frosted` | wall | (0, 0, 18) | 12 × 3 | drywall + `T_Window_GlassBricks_01.png` overlay (Reaper foreshadowing — silhouette visible) |
| `wall-end-crawford` | wall | (0, 0, 30) | 12 × 3 | drywall + `T_Window_Wood_026.png` overlay (DIRECTOR OF OPS plaque) |

### Doors

| id | kind | origin | texture | family | spawnRailId |
|---|---|---|---|---|---|
| `door-reception-side-A` | door | (-4, 0, 4) | `T_Door_Wood_010.png` | wood | `rail-spawn-reception-A` |
| `door-reception-side-B` | door | (4, 0, 4) | `T_Door_Wood_011.png` | wood | `rail-spawn-reception-B` |
| `door-lounge-side-1` | door | (-6, 0, 12) | `T_Door_Wood_018.png` | wood | `rail-spawn-lounge-1` |
| `door-lounge-side-2` | door | (6, 0, 12) | `T_Door_Wood_022.png` | wood | `rail-spawn-lounge-2` |
| `door-vent-ceiling` | door | (0, 3, 12) | `T_Door_Metal_03.png` | metal | `rail-spawn-vent-drop` |
| `door-crawford-office` | door | (0, 0, 18) | `T_Door_Wood_028.png` | wood | `rail-spawn-crawford` |

### Props & lights

| id | kind | spec |
|---|---|---|
| `prop-reception-desk` | prop | `props/desk.glb` at (0, 0, 6) |
| `prop-leather-booth-1` | prop | `traps/trap-3.glb` at (-3, 0, 12) |
| `prop-leather-booth-2` | prop | `traps/trap-3.glb` at (3, 0, 12) |
| `prop-wet-bar` | prop | `traps/trap-22.glb` at (0, 0, 14) |
| `prop-crawford-desk` | prop | `props/desk.glb` at (0, 0, 26), scale 1.4 (animated `flip` on Phase 2) |
| `prop-bourbon-glass` | prop | `traps/trap-19.glb` on Crawford desk (animated `set-down` on boss spawn) |
| `prop-pocket-watch` | prop | `traps/trap-19.glb` near Crawford |
| `light-panic-strobe` | point | (0, 2.8, 12), color (1.0, 0.3, 0.3), intensity 0.9 — strobes via cue |
| `light-crawford-spot` | spot | (0, 2.8, 26), pointing -Z, intensity 0.0 (snaps on at boss reveal) |

## Spawn rails

| id | path | speed | loop |
|---|---|---|---|
| `rail-spawn-reception-A` | (-5, 0, 5) → (-4, 0, 4.5) → (-3, 0, 4.5) | 2.5 m/s | false |
| `rail-spawn-reception-B` | (5, 0, 5) → (4, 0, 4.5) → (3, 0, 4.5) | 2.5 m/s | false |
| `rail-spawn-lounge-1` | (-7, 0, 12) → (-6, 0, 12) → (-4, 0, 12) | 2.5 m/s | false |
| `rail-spawn-lounge-2` | (7, 0, 12) → (6, 0, 12) → (4, 0, 12) | 2.5 m/s | false |
| `rail-spawn-vent-drop` | (0, 3.5, 12) → (0, 0, 12) | 6.0 m/s (drop) | false |
| `rail-spawn-pre-aggro` | (-3, 0, 14) → (-3, 0, 13) → (-2, 0, 13) | 0.5 m/s (idle pose) | false |
| `rail-spawn-crawford` | (0, 0, 30) → (0, 0, 26) | 1.5 m/s | false |
| `rail-spawn-crawford-ad-vent` | (0, 3.5, 25) → (0, 0, 25) | 6.0 m/s | false |
| `rail-spawn-crawford-ad-side` | (-6, 0, 22) → (-3, 0, 23) | 2.0 m/s | false |
...
| `pos-1` | combat | (0, 1.6, 5) | (-3, 1.6, 5) | 18000 |
| `pos-2-lounge` | combat | (0, 1.6, 11) | (0, 1.6, 14) | 22000 |
| `pos-3-crawford` | combat | (0, 1.6, 20) | (0, 1.6, 26) | 22000 |
| `exit` | glide | (0, 1.6, 22) | (0, 1.6, 24) | — |

## Cue list (screenplay)

```ts
const executiveCues: Cue[] = [
  { id: 'amb-alarm',   trigger: { kind: 'wall-clock', atMs: 0 }, action: { verb: 'ambience-fade', layerId: 'tense-drone', toVolume: 0.65, durationMs: 1500 } },
  { id: 'panic-strobe',trigger: { kind: 'wall-clock', atMs: 0 }, action: { verb: 'lighting', lightId: 'light-panic-strobe', tween: { kind: 'flicker', minIntensity: 0.3, maxIntensity: 0.9, hz: 2.5, durationMs: 75000 } } },
  { id: 'narr-floor',  trigger: { kind: 'wall-clock', atMs: 300 }, action: { verb: 'narrator', text: 'EXECUTIVE SUITES — FLOOR 47', durationMs: 1500 } },

  // Position 1 — reception
  { id: 'p1-doors',    trigger: { kind: 'on-arrive', railNodeId: 'pos-1' }, action: { verb: 'door', doorId: 'door-reception-side-A', to: 'open' } },
  { id: 'p1-doors-B',  trigger: { kind: 'on-arrive', railNodeId: 'pos-1' }, action: { verb: 'door', doorId: 'door-reception-side-B', to: 'open' } },
  { id: 'p1-spawn-A',  trigger: { kind: 'on-arrive', railNodeId: 'pos-1' }, action: { verb: 'enemy-spawn', railId: 'rail-spawn-reception-A', archetype: 'security-guard', fireProgram: 'pistol-pop-aim' } },
  { id: 'p1-spawn-B',  trigger: { kind: 'on-arrive', railNodeId: 'pos-1' }, action: { verb: 'enemy-spawn', railId: 'rail-spawn-reception-B', archetype: 'hitman', fireProgram: 'pistol-cover-pop' } },
  { id: 'p1-vault',    trigger: { kind: 'on-arrive', railNodeId: 'pos-1' }, action: { verb: 'enemy-spawn', railId: 'rail-spawn-reception-A', archetype: 'middle-manager', fireProgram: 'vault-drop-fire' } },

  // Position 2 — lounge with pre-aggro
  { id: 'p2-pre-aggro',trigger: { kind: 'on-arrive', railNodeId: 'pos-2-lounge' }, action: { verb: 'enemy-spawn', railId: 'rail-spawn-pre-aggro', archetype: 'hitman', fireProgram: 'idle' } }, // tape: idle then `pistol-pop-aim` after 2.5s
  { id: 'p2-doors-1',  trigger: { kind: 'on-arrive', railNodeId: 'pos-2-lounge' }, action: { verb: 'door', doorId: 'door-lounge-side-1', to: 'open' } },
  { id: 'p2-spawn-1',  trigger: { kind: 'on-arrive', railNodeId: 'pos-2-lounge' }, action: { verb: 'enemy-spawn', railId: 'rail-spawn-lounge-1', archetype: 'security-guard', fireProgram: 'pistol-cover-pop' } },
  { id: 'p2-doors-2',  trigger: { kind: 'on-arrive', railNodeId: 'pos-2-lounge' }, action: { verb: 'door', doorId: 'door-lounge-side-2', to: 'open' } },
  { id: 'p2-spawn-2',  trigger: { kind: 'on-arrive', railNodeId: 'pos-2-lounge' }, action: { verb: 'enemy-spawn', railId: 'rail-spawn-lounge-2', archetype: 'hitman', fireProgram: 'pistol-pop-aim' } },
  { id: 'p2-vent',     trigger: { kind: 'wall-clock', atMs: 51000 }, action: { verb: 'door', doorId: 'door-vent-ceiling', to: 'open' } },
  { id: 'p2-vent-drop',trigger: { kind: 'wall-clock', atMs: 51200 }, action: { verb: 'enemy-spawn', railId: 'rail-spawn-vent-drop', archetype: 'swat', fireProgram: 'vault-drop-fire' } },

  // Position 3 — Crawford
  { id: 'p3-amb-drop', trigger: { kind: 'on-arrive', railNodeId: 'pos-3-crawford' }, action: { verb: 'ambience-fade', layerId: 'tense-drone', toVolume: 0.20, durationMs: 800 } },
  { id: 'p3-bourbon',  trigger: { kind: 'on-arrive', railNodeId: 'pos-3-crawford' }, action: { verb: 'prop-anim', propId: 'prop-bourbon-glass', animId: 'set-down' } },
  { id: 'p3-spotlight',trigger: { kind: 'on-arrive', railNodeId: 'pos-3-crawford' }, action: { verb: 'lighting', lightId: 'light-crawford-spot', tween: { kind: 'snap', intensity: 1.5 } } },
  { id: 'p3-rack',     trigger: { kind: 'on-arrive', railNodeId: 'pos-3-crawford' }, action: { verb: 'audio-stinger', audio: 'stingers/shotgun-rack.ogg' } },
  { id: 'p3-amb-up',   trigger: { kind: 'on-arrive', railNodeId: 'pos-3-crawford' }, action: { verb: 'ambience-fade', layerId: 'tense-drone', toVolume: 0.7, durationMs: 800 } },
  { id: 'p3-door',     trigger: { kind: 'on-arrive', railNodeId: 'pos-3-crawford' }, action: { verb: 'door', doorId: 'door-crawford-office', to: 'open' } },
  { id: 'p3-boss',     trigger: { kind: 'on-arrive', railNodeId: 'pos-3-crawford' }, action: { verb: 'boss-spawn', bossId: 'crawford', phase: 1 } },

  { id: 'transition',  trigger: { kind: 'wall-clock', atMs: 75000 }, action: { verb: 'transition', toLevelId: 'boardroom' } },
];
```
