...
## Boss-tactical objects

Three shootable boss-state mutators — destructible props whose `prop-anim` cues fire boss-state changes via the director:

- **iPad** (Phase 1) — 100 HP, 5 hits to break. On break, the director cancels the redact-bar overlay for the rest of Phase 1.
- **Reaper's scythe (justice-shot during Phase 3)** — director halves the ad-spawn rate for the remainder of Phase 3 and awards 200 score. Justice-shot only; body shots ignore.

These are not pickups. They are tactical primitives that the player chooses to engage to ease the fight.
...
Single arena. 24m × 18m boardroom. 6m ceiling. Single rail position — player dwell is the entire fight.

### Floor / ceiling

| id | kind | origin | size | PBR |
|---|---|---|---|---|
| `floor-boardroom` | floor | (0, 0, 12) | 24 × 24 | `laminate` (mahogany, high gloss; blood pools tween in via `prop-anim` on Phase 3) |
| `ceiling-boardroom` | ceiling | (0, 6, 12) | 24 × 24 | `ceiling-tile`, height 6, single chandelier emissive cutout (animated `swing` via cue) + 4 vent emissive cutouts at 0.0 (vents `open` via cue) |

### Walls

| id | kind | origin | size | overlay |
|---|---|---|---|---|
| `wall-N` | wall | (-12, 0, 12) | 24 × 6 | drywall (mahogany wainscot bottom; clerestory windows top via `T_Window_Wood_028.png` overlay band) |
| `wall-S` | wall | (12, 0, 12) | 24 × 6 | drywall (mirror of N) |
| `wall-end-skyline` | wall | (0, 0, 24) | 24 × 6 | drywall + `T_Window_GlassBricks_01.png` overlay (panoramic skyline window — backlit emissive) |
| `wall-back` | wall | (0, 0, 0) | 24 × 6 | drywall + `T_Door_Wood_028.png` overlay (the 20-foot mahogany doors player just entered through) |

### Doors

| id | kind | origin | texture | family | spawnRailId |
|---|---|---|---|---|---|
| `door-entry-double` | door | (0, 0, 0) | `T_Door_DoubleDoor_011.png` | double | (none — closes behind player on entry) |
| `vent-ceiling-1` | door | (-3, 6, 8) | `T_Door_Metal_03.png` | metal | `rail-spawn-vent-1` |
| `vent-ceiling-2` | door | (3, 6, 8) | `T_Door_Metal_04.png` | metal | `rail-spawn-vent-2` |
| `vent-floor-1` | door | (-3, 0, 16) | `T_Door_Metal_05.png` | metal | `rail-spawn-floor-1` |
| `vent-floor-2` | door | (3, 0, 16) | `T_Door_Metal_06.png` | metal | `rail-spawn-floor-2` |

### Whiteboards & props

| id | kind | spec |
|---|---|---|
| `wb-q4-roadmap` | whiteboard | (-12, 1.5, 8), 4 × 1.5, "Q4 ROADMAP — REDACTED" |
| `wb-reorg-phase` | whiteboard | (12, 1.5, 16), 4 × 1.5, "REORG: PHASE Δ" |
| `prop-boardroom-table` | prop | `props/desk.glb` at (0, 0, 14), scale 4.0 (long boardroom table) |
| `prop-boardroom-chairs` | prop | `traps/trap-3.glb` instanced 16× along table edges |
| `prop-stag-head-N` | prop | `traps/trap-12.glb` at (-11.8, 4, 8) (stag-head trophy) |
| `prop-stag-head-S` | prop | `traps/trap-12.glb` at (11.8, 4, 16) |
| `prop-chandelier` | prop | `traps/trap-44.glb` at (0, 5.8, 12) (animated `swing` on Phase 2) |
| `prop-ipad` | prop | `traps/trap-19.glb` at (0, 0.8, 14) on the boardroom table |
| `prop-reaper-spot` | prop | (anchor for Reaper spawn at (0, 0, 22)) |
| `light-skyline-backlit` | hemispheric | (0, 4, 23), color (0.3, 0.4, 0.6), intensity 0.8 |
| `light-chandelier` | point | (0, 5.5, 12), color (1.0, 0.95, 0.85), intensity 0.6 |
| `light-reaper-spot` | spot | (0, 4, 22), pointing -Z, color (1.0, 0.3, 0.3), intensity 0.0 (snaps on Phase 1) |

## Spawn rails

| id | path | speed | loop |
|---|---|---|---|
| `rail-spawn-reaper-entry` | (0, 0, 26) → (0, 0, 22) | 1.0 m/s | false |
| `rail-spawn-vent-1` | (-3, 6.5, 8) → (-3, 0, 8) | 6.0 m/s | false |
| `rail-spawn-vent-2` | (3, 6.5, 8) → (3, 0, 8) | 6.0 m/s | false |
| `rail-spawn-floor-1` | (-3, -1, 16) → (-3, 0, 16) | 4.0 m/s | false |
| `rail-spawn-floor-2` | (3, -1, 16) → (3, 0, 16) | 4.0 m/s | false |
| `rail-reaper-teleport-A` | (-8, 0, 16) → (-8, 0, 16) | 0 (teleport) | false |
| `rail-reaper-teleport-B` | (8, 0, 16) → (8, 0, 16) | 0 (teleport) | false |
| `rail-reaper-teleport-C` | (-8, 0, 8) → (-8, 0, 8) | 0 (teleport) | false |
| `rail-reaper-teleport-D` | (8, 0, 8) → (8, 0, 8) | 0 (teleport) | false |

## Camera-rail nodes

A single dwell node — the boardroom is a single arena. Camera tracks the Reaper's teleport via `currentLookAt` overrides applied by the boss controller.

| id | kind | position | lookAt | dwellMs |
|---|---|---|---|---|
| `enter` | glide | (0, 1.6, -4) | (0, 1.6, 22) | — |
| `boss-arena` | combat | (0, 1.6, 8) | (0, 1.6, 22) | 60000 |
| `victory` | glide | (0, 1.6, 12) | (0, 1.6, 22) | — |

## Cue list (screenplay)

```ts
const boardroomCues: Cue[] = [
  // Approach + door swing + slam
  { id: 'amb-silence',  trigger: { kind: 'wall-clock', atMs: 0 }, action: { verb: 'ambience-fade', layerId: 'tense-drone', toVolume: 0.0, durationMs: 1000 } },
  { id: 'door-creak',   trigger: { kind: 'wall-clock', atMs: 1000 }, action: { verb: 'audio-stinger', audio: 'stingers/boardroom-door-creak.ogg' } },
  { id: 'door-open',    trigger: { kind: 'wall-clock', atMs: 1500 }, action: { verb: 'door', doorId: 'door-entry-double', to: 'open' } },
  { id: 'door-slam',    trigger: { kind: 'wall-clock', atMs: 4000 }, action: { verb: 'door', doorId: 'door-entry-double', to: 'closed' } },
  { id: 'door-thunder', trigger: { kind: 'wall-clock', atMs: 4100 }, action: { verb: 'audio-stinger', audio: 'stingers/door-thunderclap.ogg' } },
  { id: 'shake-slam',   trigger: { kind: 'wall-clock', atMs: 4100 }, action: { verb: 'camera-shake', intensity: 0.25, durationMs: 600 } },

  // Reaper spawn — Phase 1
  { id: 'reaper-spot',  trigger: { kind: 'on-arrive', railNodeId: 'boss-arena' }, action: { verb: 'lighting', lightId: 'light-reaper-spot', tween: { kind: 'snap', intensity: 1.5 } } },
  { id: 'reaper-spawn', trigger: { kind: 'on-arrive', railNodeId: 'boss-arena' }, action: { verb: 'boss-spawn', bossId: 'reaper', phase: 1 } },
  { id: 'boss-theme-1', trigger: { kind: 'on-arrive', railNodeId: 'boss-arena' }, action: { verb: 'audio-stinger', audio: 'stingers/boss-theme-phase-1.ogg' } },

  // Phase 2 → 3 transitions are emitted by director on HP thresholds; phase-bound cues below

  // Phase 2 chandelier swing (queued by director on phase-2 dispatch)
  { id: 'p2-chandelier',trigger: { kind: 'on-arrive', railNodeId: 'boss-arena' }, action: { verb: 'prop-anim', propId: 'prop-chandelier', animId: 'swing' } },

  // Vent + floor opens fire mid-Phase-3 (wall-clock-relative within boss fight)
  // (Boss controller queues these via the director's emit-cue API on Phase 3 entry.)

  // Victory
  { id: 'victory-fade', trigger: { kind: 'on-clear', railNodeId: 'boss-arena' }, action: { verb: 'audio-stinger', audio: 'stingers/reaper-death-dirge.ogg' } },
  { id: 'victory-amb',  trigger: { kind: 'on-clear', railNodeId: 'boss-arena' }, action: { verb: 'ambience-fade', layerId: 'tense-drone', toVolume: 0.0, durationMs: 3000 } },
  { id: 'transition',   trigger: { kind: 'on-clear', railNodeId: 'boss-arena' }, action: { verb: 'transition', toLevelId: 'victory' } },
];
```

## Reaper boss controller

The Reaper is the only boss with three phases (mini-bosses have two). The director's boss controller for `'reaper'`:

```
Phase 1 (HP 1500 → 1000): redact-bar volley + ad spawns from ceiling vents (sequence below)
Phase 2 (HP 1000 → 500):  teleports between A/B/C/D; scythe-slash on close
Phase 3 (HP 500 → 0):     scythe pulled; subpoena lobs + mass-pop ads from floor + ceiling
```

Phase-bound fire programs are bespoke and queued by the controller on each phase-entry. The director emits those queues into the live cue list (or via an alternative direct boss-fire API — implementation detail).
